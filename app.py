...
def predict():
    ...
    output_temp_path = "/tmp/output.mp4"
    if os.path.exists(output_temp_path):
        os.remove(output_temp_path)

    # Extract keypoints and overlay
    keypoints = extract_keypoints(video_path, output_temp_path)
    if isinstance(keypoints, dict) and 'error' in keypoints:
        return render_template('index.html', error=keypoints['error'])
    ...
    # Copy video from /tmp to static for browser access
    static_output_path = "static/output.mp4"
    shutil.copy(output_temp_path, static_output_path)
    ...
    return render_template(
        ...
        final_votes=dict(final_votes),
        final_prediction=final_prediction,
        video_url=url_for('static', filename="output.mp4") + f"?t={int(os.path.getmtime(static_output_path))}"
    )

# DO NOT USE app.run() in production on Render!
# Render will use: gunicorn app:app --bind 0.0.0.0:$PORT --timeout 120 --workers 1
